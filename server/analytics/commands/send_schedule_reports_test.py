...
mock_saved_reports = [
    {
        ...
        "params": {
            "dates": {"filter": "range", "start": "2018-06-01", "end": "2018-06-30"},
            "aggs": {
                "group": {
                    "field": "source",
                },
...
class SendScheduleReportTestCase(BaseTestCase):
    ...
    async def _test(self, report, start, end, expected_hits):
        count = 0
    ...
    async def test_send_report_hour_of_the_day(self):
        # Test exact hour
        await self._test(
        ...
        await self._test(report=report, start="2018-06-30T00", end="2018-06-30T23", expected_hits=[])
    ...
    async def test_send_report_daily(self):
        # Every day at 8am
        await self._test(
    ...
    async def test_send_report_weekly(self):
    ...
    async def test_send_report_week_days(self):
    ...
    async def test_send_report_week_ends(self):
    ...
    async def test_send_report_monthly(self):
        await self._test(
            report={"schedule": {"frequency": "monthly", "hour": 0, "day": 1}},
            start="2018-01-01T00",
            end="2018-12-31T23",
            expected_hits=[
