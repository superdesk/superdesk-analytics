...
from analytics.stats.common import (
    STAT_TYPE,
    OPERATION,
...
def process(entry, new_timeline, updates, update, stats):
    ...
    if operation in ENTER_DESK_OPERATIONS:
        ...
        if (
            updates["_current_task"].get("entered_operation") == OPERATION.MOVE
            and updates["_current_task"].get("exited_operation") == OPERATION.PUBLISH
            and updates["_current_task"]["duration"] < 3
        ):
            pass
        else:
