...
class StatsReportService(BaseReportService):
        ...
        query[must].append(
            {
                "nested": {
                    "path": "stats.timeline",
                    "query": {"terms": {"stats.timeline.task.desk": desks}},
                }
            }
        )
        ...
        query[must].append(
            {
                "nested": {
                    "path": "stats.timeline",
                    "query": {"terms": {"stats.timeline.task.user": users}},
                }
            }
        )
        ...
        query[must].append(
            {
                "nested": {
                    "path": "stats.timeline",
                    "query": {"terms": {"stats.timeline.task.stage": stages}},
                }
            }
        )
    ...
    def _es_filter_desk_transitions(self, query, value, must, params):
        ...
        if value.get("min") or value.get("max"):
            ...
            query[must].append({"range": {"num_desk_transitions": ranges}})
