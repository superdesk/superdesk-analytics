...
class PublishingPerformanceReportService(BaseReportService):
    ...
    aggregations = {"source": {"terms": {"field": "source", "size": MAX_TERMS_SIZE}}}
    ...
    def _get_aggregation_query(agg):
        ...
        query = {
            ...
            "aggs": {
                "no_rewrite_of": {
                    "filter": {"not": {"filter": {"exists": {"field": "rewrite_of"}}}},
                    "aggs": {
                        "state": {
                            "terms": {
                                "field": "state",
                ...
                "rewrite_of": {
                    "filter": {"exists": {"field": "rewrite_of"}},
                    "aggs": {
                        "state": {
                            "terms": {
                                "field": "state",
    ...
    def generate_report(self, docs, args):
        ...
        report = {"groups": {}, "subgroups": {state: 0 for state in states}}
    ...
    def generate_highcharts_config(self, docs, args):
        ...
        def gen_title():
            ...
            if len(group_keys) == 1 and report.get("subgroups"):
                data_name = chart_config.get_source_title(group_type, group_keys[0])

                return "Published Stories for {}: {}".format(group_title, data_name)
