...
class BaseReportServiceTestCase(BaseTestCase):
    ...
    def assert_bool_query(self, query, result):
        self.assertEqual(
            ...
            {
                "source": {
                    "query": {"filtered": {"filter": {"bool": result}}},
    ...
    def test_generate_elastic_query_from_date_object(self):
        ...
        query = self.service.generate_elastic_query({"params": {"dates": {"filter": "relative_hours", "relative": 12}}})
        self.assert_bool_query(
            query,
            {
                "must": [
        ...
        query = self.service.generate_elastic_query({"params": {"dates": {"filter": "relative_days", "relative": 7}}})
        self.assert_bool_query(
            query,
            {
                "must": [
    ...
    def test_generate_elastic_query_for_ingest_providers(self):
        ...
        self.assert_bool_query(
            query,
            {
                "must": [{"terms": {"ingest_provider": ["ing1", "ing2"]}}],
                "must_not": [],
            },
        )

        query = self.service.generate_elastic_query({"params": {"must_not": {"ingest_providers": ["ing1", "ing2"]}}})
        self.assert_bool_query(
            query,
            {
                "must": [],
                "must_not": [{"terms": {"ingest_provider": ["ing1", "ing2"]}}],
            },
        )
    ...
    def test_generate_elastic_query_for_content_types(self):
        ...
        query = self.service.generate_elastic_query({"params": {"must_not": {"content_types": ["text", "picture"]}}})
        self.assert_bool_query(
            query,
            {"must": [], "must_not": [{"terms": {"type": ["picture", "text"]}}]},
        )
    ...
    def test_generate_elastic_query_for_states(self):
        ...
        self.assert_bool_query(
            ...
            {
                "must": [],
                "must_not": [{"terms": {"state": ["corrected", "killed"]}}],
            },
        )
    ...
    def test_generate_elastic_query_for_sources(self):
        ...
        self.assert_bool_query(
            query,
            {"must": [{"terms": {"source": ["AAP", "Reuters"]}}], "must_not": []},
        )
        ...
        self.assert_bool_query(
            query,
            {"must": [], "must_not": [{"terms": {"source": ["AAP", "Reuters"]}}]},
        )
    ...
    def test_generate_elastic_query_size(self):
        query = self.service.generate_elastic_query({"params": {"size": 200}})
        self.assertEqual(
            query,
            {
                "source": {
